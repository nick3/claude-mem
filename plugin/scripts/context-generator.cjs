...
`+JSON.stringify(i,null,2):u=" "+this.formatData(i));let S="";if(r){let{sessionId:E,memorySessionId:A,correlationId:g,...n}=r;Object.keys(n).length>0&&(S=` {${Object.entries(n).map(([R,D])=>`${R}=${D}`).join(", ")}}`)}let O=`[${a}] [${_}] [${c}] ${l}${s}${S}${u}`;if(this.logFilePath)try{(0,j.appendFileSync)(this.logFilePath,O+`
...
`).filter(r=>r.trim()),s="";for(let r=t.length-1;r>=0;r--)try{let i=t[r];if(!i.includes('"type":"assistant"'))continue;let a=JSON.parse(i);if(a.type==="assistant"&&a.message?.content&&Array.isArray(a.message.content)){let _="";for(let c of a.message.content)c.type==="text"&&(_+=c.text);if(_=_.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g,"").trim(),_){s=_;break}}}catch(i){p.debug("PARSER","Skipping malformed transcript line",{lineIndex:r},i);continue}return{userMessage:"",assistantMessage:s}}catch(e){return p.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:d},e),{userMessage:"",assistantMessage:""}}}async function it(d,e=!1){let t=tt(),s=d?.cwd??process.cwd(),r=xe(s),i=null;try{i=new Z}catch(I){if(I.code==="ERR_DLOPEN_FAILED"){try{(0,W.unlinkSync)(et)}catch(R){p.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},R)}return p.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),""}throw I}let a=Array.from(t.observationTypes),_=a.map(()=>"?").join(","),c=Array.from(t.observationConcepts),l=c.map(()=>"?").join(","),u=i.db.prepare(`
  ...
  `).all(r,...a,...c,t.totalObservationCount),S=i.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(r,t.sessionCount+st),O="",E="";if(t.showLastMessage&&u.length>0){let I=d?.session_id,R=u.find(D=>D.memory_session_id!==I);if(R){let D=R.memory_session_id,k=rt(s),N=se.default.join((0,re.homedir)(),".claude","projects",k,`${D}.jsonl`),v=nt(N);O=v.userMessage,E=v.assistantMessage}}if(u.length===0&&S.length===0)return i?.close(),e?`
